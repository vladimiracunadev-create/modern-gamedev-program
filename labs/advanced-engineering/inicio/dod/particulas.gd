...
func compactar() -> int:
	# TODO(1): swap-remove. Recorre [0, vivas) y, cuando `vida[i] <= 0.0`,
	# decrementa `vivas`, cuenta la muerta e intercámbiala con la última viva
	# usando `_intercambiar(i, vivas)`. Cuidado: tras intercambiar NO avances
	# `i`, porque en esa posición hay ahora un elemento sin revisar.
	# Devuelve cuántas han muerto.
	return 0
