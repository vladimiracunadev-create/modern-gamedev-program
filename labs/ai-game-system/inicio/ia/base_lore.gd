...
func _visible_para(e: Dictionary, npc_id: StringName, estado: Dictionary) -> bool:
	## TODO (clase 332): tres niveles de visibilidad.
	##   - "publico"  → lo conoce cualquiera.
	##   - "npc:<id>" → solo ese NPC (el lore privado de Bram sobre su hermano).
	##   - "secreto"  → solo si `requiere_quest` está en `quests_completadas`.
	##
	## Mientras esto devuelva siempre `true`, el primer aldeano al que le
	## preguntes por la mina te destripa la trama. Compruébalo.
	return true
