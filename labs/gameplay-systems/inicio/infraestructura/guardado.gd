...
static func _m1_a_2(d: Dictionary) -> Dictionary:
	## TODO (clase 307): la v2 separó el monedero del inventario. Antes el oro
	## era un item más (`moneda_oro`) dentro de las ranuras.
	## Saca todo ese oro del inventario y crea `datos["monedero"]` con él.
	## Sin esta migración, un jugador de la v1 pierde todo su oro al actualizar.
	return d


static func _m2_a_3(d: Dictionary) -> Dictionary:
	## TODO (clase 307): la v3 renombró la habilidad "golpe" a "golpe_pesado".
	## Renombra su entrada en los cooldowns y añade el bloque `habilidades`
	## con valores por defecto si no existía.
	return d
