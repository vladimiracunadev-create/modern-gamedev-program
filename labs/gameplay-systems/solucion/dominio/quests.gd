	...
	for q in activas():
		...
		for o in q.objetivos():
			...
			if coincide and not q.requerido_completo(o):
				var clave := str(o["id"])
				var total_obj := int(o.get("cantidad", 1))
				q.progreso[clave] = mini(total_obj, int(q.progreso[clave]) + cantidad)
				progreso_objetivo.emit(q.id(), clave, int(q.progreso[clave]), total_obj)
			anteriores_ok = anteriores_ok and q.requerido_completo(o)
		if q.obligatorios_completos():
			if _transicion(q, Estado.COMPLETED):
