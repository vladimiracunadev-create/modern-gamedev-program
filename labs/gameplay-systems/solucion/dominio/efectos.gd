...
func tick(delta: float) -> void:
	...
	# Se itera sobre una COPIA de las claves: vamos a borrar mientras.
	for clave in _activos.keys().duplicate():
		...
		if e.definicion.periodo > 0.0:
			e.acumulado_tick += delta
			# 'while', no 'if': con un delta grande puede tocar más de un tick,
			# y saltárselos regalaría daño al objetivo.
			while e.acumulado_tick >= e.definicion.periodo:
				e.acumulado_tick -= e.definicion.periodo
				for efecto in e.definicion.por_tick:
					var escalado: Dictionary = efecto.duplicate()
