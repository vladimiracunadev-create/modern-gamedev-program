...
func registrar(nombre: String, campos: Dictionary = {}) -> bool:
	## TODO (clase 317): tres filtros, en este orden.
	##   1. El evento tiene que estar DECLARADO en la taxonomía. No hay
	##      eventos ad hoc.
	##   2. El consentimiento de su categoría tiene que estar dado.
	##   3. LISTA BLANCA de campos: un campo de `PROHIBIDOS` rechaza el evento
	##      ENTERO (recortarlo sería enviar la mitad de un dato personal); un
	##      campo no declarado o con valor inválido se descarta sin tumbar el
	##      evento.
