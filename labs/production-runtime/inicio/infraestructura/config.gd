...
static func validar_valor(clave: String, valor: Variant) -> String:
	## TODO (clase 315): devuelve el motivo del rechazo, o "" si es válido.
	## Hay que comprobar cuatro cosas: que la clave exista en el esquema, que
	## el tipo coincida (aceptando int donde se espera float, porque JSON no
	## los distingue), que esté dentro de min/max y que, si hay lista de
	## opciones, el valor sea una de ellas.
	## Un valor remoto malo puede hacer más daño que la falta de valor.
